...
MIDREGION_REGION = Region(20, 60, 25, 65)
MIDREGION_DATASET = 'raw/midregion_1y_1ppd.npy'
MIDREGION_DISTANCES = 'raw/distances_midregion_1y_1ppd.npy'

logger = logging.getLogger()


def save_midregion_dataset():
    '''
    Saves the SP/RJ dataset into a file
    '''
    dataset_1y_1ppd = SpatioTemporalRegion.load_1year_1ppd_last()
    midregion_region = dataset_1y_1ppd.region_subset(MIDREGION_REGION)
    midregion_region.save(MIDREGION_DATASET)


def calculate_distances():
    # load file
    midregion_dataset = np.load(MIDREGION_DATASET)
    midregion_region = SpatioTemporalRegion(midregion_dataset)

    logger.info('Calculating distances using DTW...')
    distance_measure = DistanceByDTW()
    distance_matrix = distance_measure.compute_distance_matrix(midregion_region)
    np.save(MIDREGION_DISTANCES, distance_matrix)
...
def silhouette_analysis():
    ...
    midregion_dataset = np.load(MIDREGION_DATASET)
    midregion_region = SpatioTemporalRegion(midregion_dataset)
    _, x_len, y_len = midregion_region.shape
    print(midregion_region.shape)
    ...
    weight = 0.8
    # weight = None
    save_graphs = 'plots/midregion_weight{}'.format(weight)
    ...
    distance_measure.load_distance_matrix_2d(MIDREGION_DISTANCES, MIDREGION_REGION)

    silhouette_result = kmedoids.silhouette_spt(ks, midregion_region, distance_measure,
