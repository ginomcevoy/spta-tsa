...
from spta.model.mean import TrainerMeanOfPast, MeanOfPastParams
...
# default number of samples used for training
TRAIN_SAMPLES = 10
...
def processRequest():
    ...
    # the main parser
    desc = '''MeanOfPastSolver: answer a forecast query using the mean of past training values at the medoids of
a given cluster partition. Use train sub-command to train the solver, and predict to answer prediction queries'''
...
def configure_parent(parent_parser):
    ...
    # Number of samples used to actually train the solver: calculate the mean of the last 'past'
    # values and use it as a forecast.
    help_msg = 'Number of past samples for training (default: %(default)s)'
    parent_parser.add_argument('--past', help=help_msg, default=TRAIN_SAMPLES, type=int)
    ...
    help_msg = 'Number of samples for testing (default: %(default)s)'
    parent_parser.add_argument('--test', help=help_msg, default=TEST_SAMPLES, type=int)
...
def train_request(args):
    ...
    region_metadata, clustering_metadata, model_params = metadata_from_args(args)
    ...
    model_trainer = TrainerMeanOfPast(model_params, region_metadata.x_len, region_metadata.y_len)
...
def metadata_from_args(args):
    ...
    model_params = MeanOfPastParams(args.past)

    return region_metadata, clustering_metadata, model_params
